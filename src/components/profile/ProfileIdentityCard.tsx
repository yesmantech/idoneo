...
export default function ProfileIdentityCard({ user, profile, onSettingsClick, onShareClick }: ProfileIdentityCardProps) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col items-center relative overflow-hidden group">
            ...
            <div className="relative mb-4 mt-8">
                <div className="w-28 h-28 rounded-3xl overflow-hidden border-4 border-white shadow-xl bg-white z-10 relative">
                    {avatarUrl ? (
                ...
                {/* Status Indicator */}
                <div className="absolute -bottom-2 -right-2 z-20 bg-emerald-500 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center shadow-sm" title="Online">
                    <div className="w-2.5 h-2.5 bg-white rounded-full animate-pulse"></div>
                </div>
            </div>

            {/* Identity */}
            <h1 className="text-2xl font-black text-slate-800 mb-1 text-center">{nickname}</h1>
            <div className="flex items-center gap-2 mb-4 text-center">
                <span className="text-sm font-medium text-slate-400">{user?.email}</span>
            </div>

            {/* Plan Badge */}
            <div className={`w-full py-2 rounded-xl text-xs font-bold uppercase tracking-wider border flex items-center justify-center gap-2 ${isPro
