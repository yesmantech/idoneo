...
function StatCard({ icon: Icon, iconBg, iconColor, label, value, link }: StatCardProps) {
    const content = (
        <div className="bg-[var(--card)] rounded-[20px] p-5 shadow-[0_4px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] transition-all duration-300 border border-[var(--card-border)] group">
            ...
            <div className="text-sm font-medium text-[var(--foreground)] opacity-50">{label}</div>
        </div>
    );
...
function QuickAction({ icon: Icon, iconBg, iconColor, label, description, to }: QuickActionProps) {
    return (
        <Link
            to={to}
            className="flex items-start gap-4 p-5 bg-[var(--card)] rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] transition-all duration-300 border border-[var(--card-border)] group"
        >
            <div className={`w-11 h-11 rounded-[14px] flex items-center justify-center flex-shrink-0 ${iconBg}`}>
                <Icon className={`w-5 h-5 ${iconColor}`} />
            </div>
            <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold text-[var(--foreground)] mb-0.5">{label}</div>
                <div className="text-[13px] text-[var(--foreground)] opacity-50 line-clamp-1">{description}</div>
            </div>
...
function EmptyActivityState() {
    return (
        <div className="bg-[var(--card)] rounded-[20px] p-10 shadow-[0_4px_16px_rgba(0,0,0,0.04)] border border-[var(--card-border)] text-center transition-colors">
            ...
            <p className="text-sm text-[var(--foreground)] opacity-40 max-w-sm mx-auto">
...
export default function AdminDashboardPage() {
    ...
    return (
        <AdminLayout>
            ...
            {/* Quick Actions */}
            <div className="mb-8">
                <h2 className="text-lg font-semibold text-[var(--foreground)] mb-4">Azioni rapide</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <QuickAction
                        ...
                        label="Classifiche"
                        description="Monitora i punteggi"
            ...
            {/* Recent Activity */}
            <div>
                <h2 className="text-lg font-semibold text-[var(--foreground)] mb-4">Attività recente</h2>
                <EmptyActivityState />
            </div>
        </AdminLayout>
    );
