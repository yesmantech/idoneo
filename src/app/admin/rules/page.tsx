        ...
        return (
            <AdminLayout>
                <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <h2 className="text-xl font-bold text-[var(--foreground)] mb-2">Nessun concorso selezionato</h2>
                    <p className="text-[var(--foreground)] opacity-40 mb-6">Seleziona un concorso dalla lista per configurare le regole.</p>
                    <button
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-[var(--card)] rounded-[20px] border border-[var(--card-border)] shadow-sm overflow-hidden text-[var(--foreground)]">
                    <div className="p-6 border-b border-[var(--card-border)] bg-slate-50/50 dark:bg-slate-900/50 flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold">Impostazioni Globali</h3>
                            <p className="text-sm opacity-40">Tempo e punteggi</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/10">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-xs font-medium text-emerald-600 dark:text-emerald-400 mb-1.5 flex items-center gap-1">
                                        <CheckCircle2 className="w-3 h-3" /> Esatta
                                    </label>
                                    <input
                                        type="number"
                                        step="0.01"
                                        className="w-full px-4 py-3 rounded-xl border-2 border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-bold text-center focus:outline-none focus:border-emerald-400 transition-all"
                                        value={pointsCorrect}
                                        onChange={e => setPointsCorrect(Number(e.target.value))}
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-red-600 dark:text-rose-400 mb-1.5 flex items-center gap-1">
                                        <XCircle className="w-3 h-3" /> Errata
                                    </label>
                                    <input
                                        type="number"
                                        step="0.01"
                                        className="w-full px-4 py-3 rounded-xl border-2 border-red-200 dark:border-rose-500/20 bg-red-50 dark:bg-rose-500/10 text-red-700 dark:text-rose-400 font-bold text-center focus:outline-none focus:border-red-400 transition-all"
                                        value={pointsWrong}
                                        onChange={e => setPointsWrong(Number(e.target.value))}
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium opacity-40 mb-1.5 flex items-center gap-1">
                            ...
                            <label className="flex items-center gap-3 cursor-pointer select-none">
                                <div className="relative flex items-center">
                                    <input
                                        ...
                                        className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border border-slate-300 dark:border-slate-700 transition-all checked:border-emerald-500 checked:bg-emerald-500"
                                    />
                                ...
                                <span className="text-sm font-medium opacity-60">Simulazione Ufficiale</span>
                            </label>
                        </div>
                ...
                <div className="bg-[var(--card)] rounded-[20px] border border-[var(--card-border)] shadow-sm overflow-hidden flex flex-col text-[var(--foreground)]">
                    <div className="p-6 border-b border-[var(--card-border)] bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold">Distribuzione Materie</h3>
                                <p className="text-sm opacity-40">Domande per materia</p>
                            </div>
                        </div>
                                    ...
                                    <div key={rule.subject_id} className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 group">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs opacity-40 mt-0.5">
                                                Disponibili: <span className="font-medium opacity-100">{available}</span>
                                            </p>
                                        </div>
