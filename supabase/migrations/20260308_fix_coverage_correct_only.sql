-- Fix: Coverage should count unique CORRECT answers, not just "seen" questions.
-- Before: coverage = unique_seen / total_bank
-- After:  coverage = unique_correct / total_bank
...
DECLARE
    ...
    -- Seen questions tracking (kept for data, but no longer used for coverage)
...
BEGIN
    ...
    -- Coverage = unique CORRECT / total bank (0-1) — only correctly answered questions count
...
-- =====================================================
-- RECALCULATE coverage_score for ALL existing rows
-- coverage = unique correct / total questions in quiz
-- =====================================================
UPDATE public.concorso_leaderboard lb
SET coverage_score = LEAST(
    COALESCE(jsonb_array_length(lb.correct_question_ids), 0)::numeric 
    / GREATEST((SELECT count(*) FROM public.questions q WHERE q.quiz_id = lb.quiz_id), 1)::numeric,
    1.0
);
