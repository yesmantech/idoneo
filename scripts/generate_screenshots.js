...
const screenshots = [
  ...
  },
  {
    name: '5_classifica',
    title: 'Sfida la\nClassifica',
    subtitle: 'Competi con altri studenti e scala la vetta',
...
async function generateScreenshots() {
  ...
  console.log('Assicurati di aver messo le immagini raw_{1..5}.png nella cartella scripts/raw/');
  ...
  for (const shot of screenshots) {
    ...
    if (!fs.existsSync(rawImagePath)) {
      console.log(`[SKIPPED] Immagine mancante: ${shot.image}`);
      continue;
    }
    ...
    console.log(`[FATTO] Screenshot generato: ${outPath}`);
  }

  await browser.close();
  console.log('\\n Finito! Trovi le immagini pronte per App Store nella cartella scripts/out/');
}
